...
async function setupTeable() {
    if (!config.TEABLE_API_TOKEN) {
        console.error('❌ TEABLE_API_TOKEN is missing in .env');
        process.exit(1);
    }
    ...
    try {
        ...
        try {
            ...
            console.log(`✅ Table Access Verified: ${tableId}`);
        } catch (e: any) {
            console.error(`⚠️ Could not verify table access. Check token scopes: ${e.message}`);
        }
