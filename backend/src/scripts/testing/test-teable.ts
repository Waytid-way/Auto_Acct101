...
async function testTeable() {
    console.log('๐งช Testing Teable Connection...');
    ...
    try {
        ...
        console.log('โ… Connected as:', me.data.name, `(${me.data.email})`);
        ...
        if (config.TEABLE_TABLE_ID) {
            ...
            console.log('โ… Table Found:', table.data.name);
            console.log('   Description:', table.data.description);
        }

    } catch (error: any) {
        console.error('โ Connection Failed:', {
