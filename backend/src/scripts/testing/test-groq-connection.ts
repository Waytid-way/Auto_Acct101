...
async function testGroqConnection() {
    ...
    if (!config.GROQ_API_KEY) {
        console.error('โ GROQ_API_KEY is missing');
        process.exit(1);
    }
    ...
    try {
        ...
        if (result.category.includes('Transportation') || result.category.includes('เน€เธ”เธดเธเธ—เธฒเธ')) {
            console.log('โ… Classification Logic Check: PASS');
        } else {
            console.warn('โ ๏ธ Classification Logic Check: UNEXPECTED', result.category);
        }

    } catch (error) {
        console.error('โ Groq Test Failed:', error);
    }
}
